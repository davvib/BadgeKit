...
class ViewController: NSViewController, NSTextFieldDelegate {
    ...
    func previewIcon(for item: DroppedItem) -> NSImage {
        ...
        let previewIcon = customRenderedFolderPreview(for: item, badge: badge) ??
            makeBadgedIcon(originalIcon: item.baseIconForPreview ?? item.icon, badge: badge, badgeSize: badgeSize)
    ...
    private func colorizedBitmap(from image: NSImage, size: Int, color: NSColor) -> NSBitmapImageRep? {
        ...
        let drawRect = aspectFitRect(for: image, in: NSRect(origin: .zero, size: canvasSize))
        image.draw(in: drawRect, from: .zero, operation: .sourceOver, fraction: 1.0)
        NSGraphicsContext.restoreGraphicsState()

        guard let target = color.usingColorSpace(.deviceRGB) else {
            return bitmap
        }

        var tintedPixels = 0
        let blend: CGFloat = 0.68

        for y in 0..<bitmap.pixelsHigh {
            for x in 0..<bitmap.pixelsWide {
                guard let source = bitmap.colorAt(x: x, y: y)?.usingColorSpace(.deviceRGB),
                      source.alphaComponent > 0.04 else {
                    continue
                }

                let luminance = source.redComponent * 0.299 + source.greenComponent * 0.587 + source.blueComponent * 0.114
                let shade = max(0.28, min(1.18, luminance / 0.72))
                let tinted = NSColor(
                    red: max(0, min(1, source.redComponent * (1 - blend) + target.redComponent * shade * blend)),
                    green: max(0, min(1, source.greenComponent * (1 - blend) + target.greenComponent * shade * blend)),
                    blue: max(0, min(1, source.blueComponent * (1 - blend) + target.blueComponent * shade * blend)),
                    alpha: source.alphaComponent
                )
                bitmap.setColor(tinted, atX: x, y: y)
                tintedPixels += 1
            }
        }

        if size == folderIconRenderSizes.last {
            print("Tinted folder icon pixels: \(tintedPixels)")
        }
